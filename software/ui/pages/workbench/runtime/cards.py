...
class _BenefitAreaPrefetchWorker(QObject):
    ...
    def run(self):
        try:
            from software.core.services.area_service import load_benefit_supported_areas
...
class RandomIPSettingCard(ExpandGroupSettingCard):
    ...
    def _load_area_options(self, source: Optional[str] = None):
        ...
        try:
            if source == _PROXY_SOURCE_BENEFIT:
                ...
                from software.core.services.area_service import load_area_codes
                from software.core.services.area_service import load_supported_area_codes
    ...
    def _apply_area_override(self, area_code: Optional[str]) -> None:
        from software.network.proxy.settings import apply_proxy_area_code
    ...
    def set_area_code(self, area_code: Optional[str]) -> None:
        from software.network.proxy.settings import get_proxy_settings
        if area_code is None:
    ...
    def _on_test_api_clicked(self):
        ...
        class TestWorker(QObject):
            ...
            def run(self):
                from software.network.proxy.provider import test_custom_proxy_api
    ...
    def _on_api_edit_finished(self):
        """API地址输入完成时同步到全局变量"""
        from software.network.proxy.settings import apply_custom_proxy_api
        api_url = self.customApiEdit.text().strip()
